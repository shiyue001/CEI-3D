...
def evaluate(**kwargs):
    ...
    evaldir = os.path.join('../iccv23/exps', task, kwargs['expname'], flag, os.path.basename(kwargs['data_split_dir']))
    ...
    # envmapmaterial_ckpt_path = os.path.join(str(kwargs['model_params_dir']) + "envmap_material_network.pth")
    # envmapmaterial_state = torch.load(envmapmaterial_ckpt_path)["model_state_dict"]

    # unedited_diffuse_ckpt_path = os.path.join(str(kwargs['model_params_dir']) + "unedited_diffuse_albedo_layer.pth")
    # unedited_diffuse_state = torch.load(unedited_diffuse_ckpt_path)["model_state_dict"]

    # edited_diffuse_ckpt_path = os.path.join(str(kwargs['model_params_dir']) + "unedited_diffuse_albedo_layer.pth")
    # unedited_diffuse_state = torch.load(unedited_diffuse_ckpt_path)["model_state_dict"]
    ...
    # for ks in saved_model_state["model_state_dict"].keys():
    #     if ks.startswith('envmap_material_network.diffuse'):
    #         print(ks)

    #################################################################################################
    ### load model parameters, unedited mlp and edited mlp are carefully loaded here
