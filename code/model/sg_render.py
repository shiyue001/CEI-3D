...
def render_with_sg(lgtSGs, specular_reflectance, roughness, diffuse_albedo, normal, viewdirs, blending_weights=None, diffuse_rgb=None):
    ...
    if diffuse_rgb is None:
        # diffuse_albedo[:1000,0] = 0.4
        # diffuse_albedo[:1000,1] = 0
        # diffuse_albedo[:1000,2] = 0
        ...
        # print(diffuse_rgb.shape, diffuse_rgb.min().item(), diffuse_rgb.max().item())

        # diffuse_rgb[:1000,0] = 0.4
        # diffuse_rgb[:1000,1] = 0
        # diffuse_rgb[:1000,2] = 0

        # diffuse_rgb = diffuse_albedo


        # diffuse_rgb = torch.zeros_like(diffuse_rgb)

    # combine diffue and specular rgb, then return
    # print("diffuse_rgb: ", diffuse_rgb)
    ...
    # print(type(diffuse_albedo), diffuse_albedo.shape)
