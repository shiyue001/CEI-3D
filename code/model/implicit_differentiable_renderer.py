...
class ImplicitNetwork(nn.Module):
    ...
    #input.shape--torch.Size([10000, 3])
...
class IDRNetwork(nn.Module):
    ...
    # we need to change the implicit_network in forward, if we want to change to another geometry network
    def forward(self, input):
        ...
        #set the mode of implicit_network to eval
        ...
        #cam_loc--torch.Size([1, 3]); dists--torch.Size([10000]); ray_dirs--torch.Size([1, 10000, 3])
        points = (cam_loc.unsqueeze(1) + dists.reshape(batch_size, num_pixels, 1) * ray_dirs).reshape(-1, 3) # 坐标转换
        #points.shape--torch.Size([10000, 3])

        sdf_output = self.implicit_network(points)[:, 0:1] # 通过MLP输出物体几何
        #sdf_output.shape--torch.Size([10000, 1]), value range: 0~1
        # print('sdf output shape: ', sdf_output.shape)

        # print(sdf_output[network_object_mask & object_mask])

        # exit()

        ray_dirs = ray_dirs.reshape(-1, 3)
        #ray_dirs--torch.Size([10000, 3])
        #skip(eval)
        if self.training:
        ...
        #eval,here
            ...
            surface_mask = network_object_mask  #torch.Size([10000]),true or false
            differentiable_surface_points = points[surface_mask] # 物体表面点 torch.Size([0, 3]), null??
            diffuse_rgb = diffuse_rgb[surface_mask] if diffuse_rgb is not None else None #null
            grad_theta = None

        #points.shape--torch.Size([10000, 3])
        #the following code are initializing the attributes of points
    ...
    def forward_idr(self, input):
        ...
        if differentiable_surface_points.shape[0] > 0:
            rgb_values[surface_mask] = self.get_rbg_value_idr(differentiable_surface_points, view)
    ...
    def get_rbg_value_idr(self, points, view_dirs):
        output = self.implicit_network(points)
