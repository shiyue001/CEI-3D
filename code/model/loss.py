...
class IDRLoss(nn.Module):
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        idr_rgb_loss, sg_rgb_loss = self.get_rgb_loss(model_outputs['idr_rgb_values'], model_outputs['sg_rgb_values'],
                                                      rgb_gt, network_object_mask, object_mask) # sg_rgb_loss
...
class IDRLoss1(nn.Module):
    def __init__(self, eikonal_weight, mask_weight, alpha, loss_type='L1'):
        super().__init__()
    ...
    def get_rgb_loss(self, idr_rgb_values, rgb_gt, network_object_mask, object_mask):
        ...
        idr_rgb_loss = self.img_loss(idr_rgb_values, rgb_gt) / float(object_mask.shape[0])
    ...
    def forward(self, model_outputs, ground_truth):
        ...
        idr_rgb_loss = self.get_rgb_loss(model_outputs['rgb_values'], rgb_gt, network_object_mask, object_mask)
        mask_loss = self.get_mask_loss(model_outputs['sdf_output'], network_object_mask, object_mask)
        eikonal_loss = self.get_eikonal_loss(model_outputs['grad_theta'])

        # print(idr_rgb_loss, mask_loss, eikonal_loss)
