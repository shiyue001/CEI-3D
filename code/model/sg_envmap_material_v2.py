...
### modified by shirui at 2022.10.11
### split EnvmapMaterialNetwork into diffuse_albedo_layer and others
...
class EnvmapMaterialNetwork_v2(nn.Module):
...
class Diffuse_albedo_layers(nn.Module):
    def __init__(self, multires, dims=[512, 512, 512, 512]):
    ...
    def forward(self, points):
        if points is None:
            ...
            x = self.layer_0(points)
            x = self.layer_1(x)
            x = self.layer_2(x)
            code = self.layer_3(x)
            diffuse_albedo = self.layer_4(code)
            diffuse_albedo = torch.sigmoid(diffuse_albedo)

            # print('diffuse_albedo shape: ', diffuse_albedo.shape)

        return diffuse_albedo

    def forward_code(self, points):
        if points is None:
            code = None
        else:
            if self.embed_fn is not None:
    ...
    def forward_diffuse_albedo(self, code):
        if code is None:
            diffuse_albedo = None
        else:
            diffuse_albedo = self.layer_4(code)
