...
if args.dataset_path != "":
    from nerf_synthetic import NeRFSynthetic, mNeRFSynthetic
    data = mNeRFSynthetic(args.dataset_path, split=args.dataset_split, testskip=args.dataset_skip)
    # if args.pcd_path != "":
...
while True:
    ...
    if key == ord("p"):
        ...
        if args.dataset_path != "":
            ...
            original_image = cv2.cvtColor(original_image_rgb, cv2.COLOR_RGB2BGR)
            if pnt_w is not None:
                # pnt_w is the coords of point clouds
                uv_cam, pnt_cam, depth = project_pcd(pnt_w, data.K, c2w)
                depth_map, index = get_depth_map(uv_cam, depth, *original_image_rgb.shape[:2], scale=3)
    ...
    if key == ord("c") and pnt_w is not None and mask is not None:
        # import pdb; pdb.set_trace()
        if depth_ratio == 100:
            # mask is 2d mask; pnt_frame_mask.shape--(34960, 1); uv_cam.shape--(34960, 3)
