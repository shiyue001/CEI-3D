...
from deepsdf_dataset import SDFSamples, GeometryEditingColor
...
def finetune(**kwargs):
    ...
    implicit_network.load_state_dict(implicit_network_dict, strict=True)
    ...
    #######################################################################################################
    ## load dataset
    # dataset = GeometryEditingColor(data_source='/data2/code_backup/PhySG/code/geometry_editing/data/dtu_scan110/modified.ply', subsample=10000)
    dataset = GeometryEditingColor(data_source='/data2/code_backup/PhySG/code/geometry_editing/data/physg_bear/deformed_mesh_color.obj', subsample=10000)
    ...
    # diffuse_albedo_layer_optimizer = torch.optim.SGD(diffuse_albedo_layer.parameters(),
    ...
    for epoch in range(n_epochs):
        # print('Epoch: ', epoch)
        diffuse_albedo_layer.train()

        for data_index, (input, gt_color, _) in enumerate(train_dataloader):
            ...
            gt_color = gt_color.float().cuda()


            # exit()
            diffuse_albedo_layer_optimizer.zero_grad()

            output = diffuse_albedo_layer(input)

            pred_color = output['sg_diffuse_albedo']



            # print(gt_color.shape, pred_color.shape)
            # print(gt_color[0, 0:1, :], pred_color[0, 0:1, :])

            # [:, 0:1]
            # print(pred_color.shape, gt_color.shape)
            loss = loss_l1(pred_color, gt_color)
    ...
    ## save diffuse albedo layer
    ckpt_save_dir = '/data2/code_backup/PhySG/code/geometry_editing/output/physg_bear/ckpt'
    # ckpt_save_dir = '/data2/code_backup/PhySG/code/geometry_editing/output/nerf_chair_arap'
    # ckpt_save_dir = '/data2/code_backup/PhySG/code/geometry_editing/data/shape09108_rank00/ckpt'
    torch.save({"model_state_dict": diffuse_albedo_layer.state_dict()}, os.path.join(ckpt_save_dir, "diffuse_albedo_layer_finetuned.pth"))
