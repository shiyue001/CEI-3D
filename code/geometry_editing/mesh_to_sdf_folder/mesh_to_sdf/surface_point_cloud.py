...
class SurfacePointCloud:
    ...
    def sample_sdf_near_surface(self, number_of_points=500000, use_scans=True, sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False, sampling_pnts=None):
        
        query_points = []
        if sampling_pnts is not None:
            print('use provided sampling pnts')
            surface_points = sampling_pnts
            query_points.append(surface_points)
            # unit_sphere_sample_count = number_of_points - surface_points.shape[0]
            number_of_points -= surface_points.shape[0]
...
def sample_from_mesh(mesh, sample_point_count=10000000, calculate_normals=True):
    ...
    return points
    #yue change the return on 06/05/2024
    # return SurfacePointCloud(mesh, 
    #     points=points,
