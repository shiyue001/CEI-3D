...
from utils.plots_idr import plot_idr
...
class IDRTrainRunner():
    def __init__(self,**kwargs):
        ...
        self.nepochs_idr = kwargs['nepochs_idr']
        ...
        self.train_dataset = utils.get_class(self.conf.get_string('train_physg.dataset_class'))(kwargs['gamma'],
        ...
        self.plot_dataset = utils.get_class(self.conf.get_string('train_physg.dataset_class'))(kwargs['gamma'],
                                            kwargs['data_split_dir'], self.train_cameras)
        ...
        self.model = utils.get_class(self.conf.get_string('train_physg.model_class'))(conf=self.conf.get_config('model'))
        # self.model_idr = utils.get_class(self.conf.get_string('train.model_idr_class'))(conf=self.conf.get_config('model_idr'))


        ### idr parameter
        self.lr_idr = self.conf.get_float('train_idr.idr_learning_rate')
        self.optimizer_idr = torch.optim.Adam(list(self.model.implicit_network.parameters()) + list(self.model.rendering_network.parameters()), 
                                        lr=self.lr_idr)
        self.sched_milestones_idr = self.conf.get_list('train_idr.idr_sched_milestones', default=[])
        self.sched_factor_idr = self.conf.get_float('train_idr.idr_sched_factor', default=0.0)
        self.scheduler_idr = torch.optim.lr_scheduler.MultiStepLR(self.optimizer_idr, self.sched_milestones_idr, gamma=self.sched_factor_idr)

        if torch.cuda.is_available():
            self.model.cuda()

        self.loss = utils.get_class(self.conf.get_string('train_physg.loss_class'))(**self.conf.get_config('loss'))
        self.idr_loss = utils.get_class(self.conf.get_string('train_physg.idr_loss_class'))(**self.conf.get_config('idr_loss'))
        ...
        self.idr_optimizer = torch.optim.Adam(list(self.model.implicit_network.parameters()) + list(self.model.rendering_network.parameters()),
                                              lr=self.conf.get_float('train_physg.idr_learning_rate'))
        self.idr_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.idr_optimizer,
                                                              self.conf.get_list('train_physg.idr_sched_milestones', default=[]),
                                                              gamma=self.conf.get_float('train_physg.idr_sched_factor', default=0.0))

        self.sg_optimizer = torch.optim.Adam(self.model.envmap_material_network.parameters(),
                                              lr=self.conf.get_float('train_physg.sg_learning_rate'))
        self.sg_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.sg_optimizer,
                                                              self.conf.get_list('train_physg.sg_sched_milestones', default=[]),
                                                              gamma=self.conf.get_float('train_physg.sg_sched_factor', default=0.0))
        ...
        if self.train_cameras:
            ...
            self.optimizer_cam = torch.optim.SparseAdam(self.pose_vecs.parameters(), self.conf.get_float('train_physg.learning_rate_cam'))
        ...
        self.num_pixels = self.conf.get_int('train_physg.num_pixels')
        self.total_pixels = self.train_dataset.total_pixels
        self.img_res = self.train_dataset.img_res
        self.n_batches = len(self.train_dataloader)
        self.plot_freq = self.conf.get_int('train_physg.plot_freq')
        self.plot_conf = self.conf.get_config('plot')


        self.ckpt_freq = self.conf.get_int('train_physg.ckpt_freq')

        self.alpha_milestones = self.conf.get_list('train_physg.alpha_milestones', default=[])
        self.alpha_factor = self.conf.get_float('train_physg.alpha_factor', default=0.0)
        for acc in self.alpha_milestones:
            if self.start_epoch * self.n_batches > acc:
                self.loss.alpha = self.loss.alpha * self.alpha_factor

        ### idr parameters
        self.plot_idr_conf = self.conf.get_config('plot_idr')
        self.plot_freq_idr = self.conf.get_int('train_idr.plot_freq')
        self.ckpt_freq_idr = self.conf.get_int('train_idr.ckpt_freq')
        self.alpha_milestones_idr = self.conf.get_list('train_idr.alpha_milestones', default=[])
        self.alpha_factor_idr = self.conf.get_float('train_idr.alpha_factor', default=0.0)
    ...
    def run_idr(self):
        ...
        for epoch in range(self.nepochs_idr + 1):

            if epoch in self.alpha_milestones_idr:
                self.idr_loss.alpha = self.idr_loss.alpha * self.alpha_factor_idr

            if epoch % 100 == 0:
                self.save_checkpoints(epoch)

            if epoch % self.plot_freq_idr == 0:
                self.model.eval()
                if self.train_cameras:
                    self.pose_vecs.eval()
                self.train_dataset.change_sampling_idx(-1)
                ...
                split = utils.split_input_idr(model_input, self.total_pixels)
                res = []
                for s in split:
                    out = self.model.forward_idr(s)
                    res.append({
                        'points': out['points'].detach(),
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                model_outputs = self.model.forward_idr(model_input)
                loss_output = self.idr_loss(model_outputs, ground_truth)

                loss = loss_output['loss']

                self.optimizer_idr.zero_grad()
                if self.train_cameras:
                    self.optimizer_cam.zero_grad()

                loss.backward()

                self.optimizer_idr.step()
                if self.train_cameras:
                    self.optimizer_cam.step()
                ...
                print(
                    '{0} [{1}] ({2}/{3}): loss = {4}, rgb_loss = {5}, eikonal_loss = {6}, mask_loss = {7}, alpha = {8}, lr = {9}'
                        .format(self.expname, epoch, data_index, self.n_batches, loss.item(),
                                ...
                                loss_output['mask_loss'].item(),
                                self.idr_loss.alpha,
                                self.scheduler_idr.get_lr()[0]))

            self.scheduler_idr.step()
